...
if(!class_exists("FacebookLiveVideoAutoEmbedBase")) {
	class FacebookLiveVideoAutoEmbedBase {
    	public $key = "727BCDF0355665F8";
    	private $product_id = "1";
    	private $product_base = "fb-live-video-autoembed";
    	private $server_host = "https://www.sleekalgo.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_fb-live-video-autoembed_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=fb-live-video-autoembed_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
